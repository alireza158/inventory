...
class CommissionReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $filteredQuery = CommissionResult::query()->with(['period', 'user', 'category']);
        ...
        if ($periodId) {
            $filteredQuery->where('commission_period_id', $periodId);
        }
        if ($userId) {
        ...
        $dashboardBaseQuery = (clone $filteredQuery)->reorder();

        $dashboard = [
            'total_sold_amount' => (clone $dashboardBaseQuery)->sum('sold_amount'),
            'total_commission_amount' => (clone $dashboardBaseQuery)->sum('commission_amount'),
            'best_user' => (clone $dashboardBaseQuery)
                ->selectRaw('user_id, SUM(commission_amount) as total')
                ->groupBy('user_id')
                ->orderByDesc('total')
                ->with('user:id,name')
                ->first(),
            'weakest_category' => (clone $dashboardBaseQuery)
                ->selectRaw('category_id, AVG(achievement_percent) as avg_achievement')
                ->groupBy('category_id')
                ->orderBy('avg_achievement')
                ->with('category:id,name')
                ->first(),
            'overall_achievement_percent' => round((float) (clone $dashboardBaseQuery)->avg('achievement_percent'), 2),
        ];
