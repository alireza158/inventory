...
<div class="finance-report-page">
  ...
  <div class="card report-card report-filter-card mb-3 no-report-print">
    <div class="card-body">
      <form id="salesVisitorsFilterForm" class="row g-3 align-items-end" method="GET" action="{{ route('finance.reports.sales-visitors') }}">
        ...
        <div class="col-sm-6 col-xl-3"><label class="form-label">وضعیت فاکتور</label><select class="form-select" name="status">@foreach($statusOptions as $value => $label)<option value="{{ $value }}" @selected(($filters['status'] ?? 'commissionable') === (string) $value)>{{ $label }}</option>@endforeach</select></div>
  ...
  <form method="POST" action="{{ route('finance.reports.sales-visitors.commission-batches.store') }}" class="card report-card no-report-print" id="commissionBatchForm">
    ...
    <div class="card-header bg-white d-flex justify-content-between align-items-center flex-wrap gap-2">
      ...
      <div class="small text-muted">برای ثبت batch، انتخاب یک ویزیتور مشخص الزامی است.</div>
    </div>
    ...
    <div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th><input type="checkbox" id="selectAllInvoices"></th><th>شماره فاکتور</th><th>تاریخ</th><th>نام مشتری</th><th>موبایل</th><th>ویزیتور/ثبت‌کننده</th><th class="text-end">مبلغ فاکتور</th><th class="text-end">تخفیف</th><th class="text-end">مبلغ نهایی</th><th>وضعیت</th><th>وضعیت پورسانت</th></tr></thead><tbody>@forelse($details as $invoice)@php $alreadyApproved = in_array((int) $invoice->id, $approvedInvoiceIds, true); $commissionable = in_array($invoice->status, [\App\Models\Invoice::STATUS_SHIPPED, \App\Models\Invoice::STATUS_FINANCE_APPROVED], true); $canSelect = !$alreadyApproved && $commissionable && ($filters['visitor_id'] ?? '') !== ''; @endphp<tr><td><input type="checkbox" class="invoice-check" name="invoice_ids[]" value="{{ $invoice->id }}" data-total="{{ (int) $invoice->total }}" @disabled(!$canSelect)></td><td><a class="code-cell" href="{{ route('invoices.show', $invoice->uuid) }}">{{ $invoice->uuid }}</a></td><td>{{ $jalali($invoice->display_document_date) }}</td><td>{{ $invoice->customer_name ?: '—' }}</td><td>{{ $invoice->customer_mobile ?: '—' }}</td><td>{{ $invoice->preinvoiceOrder?->creator?->name ?: 'نامشخص' }}</td><td class="money-cell">{{ $rial($invoice->subtotal) }}</td><td class="money-cell">{{ $rial($invoice->discount_amount) }}</td><td class="money-cell fw-bold">{{ $rial($invoice->total) }}</td><td>{{ $statusLabels[$invoice->status] ?? ($invoice->status ?: '—') }}</td><td>@if($alreadyApproved)<span class="badge text-bg-secondary">قبلاً تأیید شده</span>@elseif(!$commissionable)<span class="badge text-bg-warning text-dark">غیرقابل انتخاب</span>@elseif(($filters['visitor_id'] ?? '') === '')<span class="badge text-bg-info">ابتدا ویزیتور را انتخاب کنید</span>@else<span class="badge text-bg-success">قابل انتخاب</span>@endif</td></tr>@empty<tr><td colspan="11" class="text-center text-muted py-4">فاکتوری یافت نشد.</td></tr>@endforelse</tbody></table></div>
