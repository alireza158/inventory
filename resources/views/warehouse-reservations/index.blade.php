...
<div class="container-fluid" dir="rtl">
    ...
    <form class="card border-0 shadow-sm mb-3" method="GET" action="{{ route('warehouse.reservations.index') }}">
        <div class="card-body row g-2 align-items-end">
            ...
            <div class="col-md-2"><label class="form-label">مشتری</label><select name="customer_id" class="form-select"><option value="">همه</option>@foreach($customers as $customer)<option value="{{ $customer->id }}" @selected(($filters['customer_id'] ?? '') == $customer->id)>{{ $customer->display_name ?: $customer->mobile }}</option>@endforeach</select></div>
