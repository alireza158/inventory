    ...
    <div class="card"><div class="card-body">
        <form method="POST" action="{{ route('vouchers.section.store', 'personnel') }}">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4"><label class="form-label">تحویل‌گیرنده</label><input type="text" class="form-control form-control-sm mb-1 user-select-filter" data-target="receiverUserSelect" placeholder="جستجو بر اساس نام، تلفن، ایمیل یا کد پرسنلی"><select id="receiverUserSelect" name="receiver_user_id" class="form-select" required><option value="">انتخاب پرسنل...</option>@foreach($receiverUsers as $user)<option value="{{ $user->id }}" data-search="{{ trim($user->name.' '.$user->phone.' '.$user->email.' '.$user->personnel_code) }}" @selected((string) old('receiver_user_id') === (string) $user->id)>{{ $user->name }}{{ $user->phone ? ' - '.$user->phone : '' }}</option>@endforeach</select></div>
