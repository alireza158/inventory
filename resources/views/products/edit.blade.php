...
<script>
const modelOptions = @json($modelListOptions); let idx=0; function code12(c,m,v){return `${c||'0000'}${String(m||'').padStart(4,'0')}${v||'0000'}`}
function addVariantRow(data={}){const tb=document.querySelector('#variantsTable tbody');const i=idx++;const opts=modelOptions.map(m=>`<option value="${m.id}" data-code="${m.code}" ${String(data.model_list_id||'')===String(m.id)?'selected':''}>${m.brand ? (m.brand + ' - ') : ''}${m.model_name} (${m.code||'---'})</option>`).join('');const tr=document.createElement('tr');tr.innerHTML=`<td><input type="hidden" name="variants[${i}][id]" value="${data.id||''}"><select class="form-select model" name="variants[${i}][model_list_id]" required><option value="">انتخاب</option>${opts}</select></td><td><input class="form-control vname" name="variants[${i}][variant_name]" value="${data.variant_name||''}" required></td><td><input class="form-control variety" maxlength="4" name="variants[${i}][variety_code]" value="${data.variety_code||''}" required></td><td><input class="form-control final" readonly><input type="hidden" class="variety-name" name="variants[${i}][variety_name]" value="${data.variety_name||''}"></td><td><input class="form-control" type="number" name="variants[${i}][sell_price]" value="${data.sell_price||0}" min="0" required></td><td><input class="form-control" type="number" name="variants[${i}][buy_price]" value="${data.buy_price||''}" min="0"></td><td><input class="form-control" type="number" name="variants[${i}][stock]" value="${data.stock||0}" min="0" required></td><td><input type="hidden" name="variants[${i}][is_active]" value="0"><div class="form-check form-switch"><input class="form-check-input" type="checkbox" name="variants[${i}][is_active]" value="1" ${data.is_active === false || Number(data.is_active) === 0 ? '' : 'checked'}></div></td><td><button type="button" class="btn btn-sm btn-outline-danger" onclick="this.closest('tr').remove()">×</button></td>`;tb.appendChild(tr);tr.addEventListener('input',()=>refreshRow(tr));tr.addEventListener('change',()=>refreshRow(tr));refreshRow(tr)}
